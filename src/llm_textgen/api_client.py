...
class OpenAICompatibleTextGenerator:
    ...
    async def _generate_many_async(self, prompts: Sequence[str]) -> List[str]:
        ...
        async with httpx.AsyncClient(limits=limits, timeout=timeout, http2=True) as client:
            ...
            async def _task(index: int, prompt: str) -> str:
                async with sem:
    ...
    def generate_many(self, prompts: Sequence[str]) -> List[str]:
        ...
        if not prompts:
            return []
        return asyncio.run(self._generate_many_async(list(prompts)))
